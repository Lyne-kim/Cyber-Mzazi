...
Cyber Mzazi is a consent-led child online safety platform that enables parents or guardians to monitor and review potentially harmful digital messages affecting a child. The system provides a Flask web application, a MySQL database, a machine-learning classifier, curated safety resources, an AI Safety Assistant, and a child-only Android companion app. Parents can register families, manage child profiles, review or delete alerts, approve or deny child logout requests, and create Android device pairing links. Children can report messages through the web platform or child APK, while the Android companion can ingest supported notification text from a child phone after pairing and permission setup.
...
- Figure 3.2: Android child-device pairing workflow
...
- To build a child-only Android companion app for paired notification ingestion and child-safe controls.
...
- To provide parent alert review, alert deletion, feedback correction, logout approval, curated safety resources, and AI guidance features.
...
- Android companion app: The native Android child application used on a paired child device.
...
- backend API testing
- child-only Android APK testing
...
The latest stage-3 retraining used only the dataset folder:
...
- normalized dataset rows: 1,298
- validation support: 260 rows
- validation accuracy after epoch 4: 0.673
- validation macro F1 after epoch 4: 0.720
- validation loss after epoch 4: 0.8709
- device: CPU
- epochs: 4
- batch size: 8
- max length: 160
...
where `y` belongs to one of the ten active supported labels.
...
For `K = 10` classes, logits are computed as:
...
The current product direction keeps the Android APK child-only while parents use the web dashboard for alert review, family management, safety resources, AI assistant guidance, and approval workflows.
